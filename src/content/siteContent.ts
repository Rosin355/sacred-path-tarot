...
export const sitePages = ["home", "arcani", "respiro", "ispirazione", "chi_sono"] as const;
...
export const contentFields: Record<SitePage, ContentField[]> = {
  home: [
    ...
    f("arcani_brand", "Marchio Tarocchi", "Via degli Arcani", "title"),
    ...
    f("respiro_brand", "Marchio Yoga", "Via del Respiro", "title"),
  ...
  arcani: [
    ...
    f("hero_brand", "Marchio Tarocchi", "Apertura", "title"),
  ...
  respiro: [
    f("hero_kicker", "Titolo breve", "Apertura", "title"), f("hero_title", "Titolo", "Apertura", "title"), f("hero_text", "Introduzione", "Apertura", "paragraph"),
    f("hero_brand", "Marchio Yoga", "Apertura", "title"),
  ...
  chi_sono: [
    f("kicker", "Titolo breve", "Presentazione", "title"), f("title", "Titolo principale", "Presentazione", "title"),
    f("intro", "Introduzione", "Presentazione", "paragraph"),
    f("biography_1", "Biografia · primo paragrafo", "Biografia", "paragraph"),
    f("biography_2", "Biografia · secondo paragrafo", "Biografia", "paragraph"),
  ],
};

export const defaultSiteContent: Record<SitePage, SitePageContent> = {
  home: {
    hero_kicker: "Percorsi con Jessica Marin", hero_title: "Tre Vie Per Illuminarsi", hero_subtitle: "Tarocchi, yoga, attività fisica e arte: tre vie per conoscerti, ritrovare equilibrio ed esprimere ciò che sei.",
    ...
    arcani_kicker: "Il cammino attraverso i simboli", arcani_title: "La Via degli Arcani", arcani_brand: "𝐓𝐚𝐫𝐨𝐜𝐜𝐡𝐢𝓟𝓮𝓻𝐈𝐥𝐥𝐮𝐦𝐢𝐧𝐚𝐫𝐬𝐢✨", arcani_intro: "Non solo divinazione: i tarocchi come via di conoscenza, interpretazione e consapevolezza.",
    ...
    respiro_kicker: "Yoga e attività fisica", respiro_title: "La Via del Respiro", respiro_brand: "𝐘𝐨𝐠𝐚𝓟𝓮𝓻𝐈𝐥𝐥𝐮𝐦𝐢𝐧𝐚𝐫𝐬𝐢✨", respiro_intro: "La pratica è il luogo in cui il corpo ricorda ciò che la mente dimentica: Yoga e attività fisica si incontrano in un percorso di ascolto, forza e consapevolezza.", respiro_group_1: "Attività fisica e pratiche dinamiche", respiro_items_1: "Power Yoga · Ginnastica Total Body", respiro_group_2: "Yoga, tecnica e respiro", respiro_items_2: "Iyengar · Hatha · Yin · Pranayama", respiro_closing: "Equilibrio tra flessibilità, forza, potenza e resistenza; atmosfera e ascolto, con chiusura di rilassamento e integrazione.", respiro_place: "Kairos Spazio Olistico — mercoledì 20:30–21:45", respiro_note: "+ sostituzioni in altre palestre",
  ...
  arcani: {
    hero_kicker: "Il cammino attraverso i simboli", hero_title: "La Via degli Arcani", hero_brand: "𝐓𝐚𝐫𝐨𝐜𝐜𝐡𝐢𝓟𝓮𝓻𝐈𝐥𝐥𝐮𝐦𝐢𝐧𝐚𝐫𝐬𝐢✨", hero_text: "La Via degli Arcani è il percorso dedicato a chi desidera entrare davvero nel linguaggio dei tarocchi, non solo come strumento divinatorio, ma come via di conoscenza, interpretazione e consapevolezza. Qui Jessica Marin accompagna l'allievo nello studio degli arcani maggiori, degli arcani minori, dei metodi di stesura, della medianità attraverso i tarocchi e della carta del destino.",
  ...
  chi_sono: {
    kicker: "La Sacerdotessa", title: "Chi sono",
    intro: "Sono Jessica Marin. Ti accompagno nell’esplorazione del tuo mondo interiore attraverso simboli, tarocchi e pratiche del corpo.",
    biography_1: "Nei miei corsi, percorsi e workshop condivido conoscenze esoteriche, yoga e attività motorie, dai livelli di base a quelli avanzati.",
    biography_2: "Credo che ogni persona possa trovare la propria luce interiore attraverso un cammino di ascolto, studio e consapevolezza. Scegli la via che senti tua.",
  },
};
...
export const mergeWithDefaults = (page: SitePage, value: unknown): SitePageContent => {
  if (!value || typeof value !== "object" || Array.isArray(value)) return defaultSiteContent[page];
  const stored = value as Record<string, unknown>;
  const candidate = Object.fromEntries(contentFields[page].map(field => {
    const current = stored[field.key];
    const valid = typeof current === "string" && current.trim().length > 0 && current.length <= maxLengthFor(field.kind) && !/<[^>]+>/.test(current);
    return [field.key, valid ? current : defaultSiteContent[page][field.key]];
  }));
  const parsed = pageContentSchema(page).safeParse(candidate);
