  ...
  return <section id="tarot-section" className="py-32 lg:py-40 px-6 lg:px-12 relative">
      <div className="container mx-auto max-w-7xl">
        <div className="mb-24 space-y-6 animate-fade-in">
          ...
          <p className="text-muted-foreground max-w-2xl leading-relaxed font-light text-2xl">
        ...
        <Card className="bg-card/30 backdrop-blur-sm minimal-border">
          <CardHeader className="p-8">
            <CardTitle className="font-display tracking-tight text-5xl">
              Scegli le Tue Carte
            </CardTitle>
            <CardDescription className="font-light text-xl">
          ...
          <CardContent className="space-y-12 p-8 pt-0">
            {/* Card Selection */}
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-7 gap-4" onMouseMove={handleDragMove} onMouseUp={handleDragEnd} onMouseLeave={handleDragEnd} onTouchMove={handleDragMove} onTouchEnd={handleDragEnd}>
              ...
              return <div key={index} ref={el => cardRefs.current[index] = el} onMouseDown={e => handleDragStart(e, index)} onTouchStart={e => handleDragStart(e, index)} className={`
                      relative aspect-[2/3] minimal-border overflow-hidden cursor-grab
                      ${selectedCards.includes(index) ? "ring-2 ring-accent" : "opacity-60 hover:opacity-100"}
                      ${isReading ? "pointer-events-none" : ""}
                      ${isDragging ? "cursor-grabbing z-50" : ""}
                    `} style={{
                transform: isDragging ? `translate3d(${translateX}px, ${translateY}px, 0) scale(1.1)` : selectedCards.includes(index) ? "scale(1.05)" : "scale(1)",
                transition: isDragging ? "none" : "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",
                willChange: isDragging ? "transform" : "auto",
                touchAction: "none"
              }}>
                    <img src={tarotCardBack} alt={`Carta ${index + 1}`} className="w-full h-full object-cover pointer-events-none select-none" draggable="false" />
                    {selectedCards.includes(index) && <div className="absolute inset-0 bg-accent/10 backdrop-blur-[1px] flex items-center justify-center pointer-events-none">
            ...
            {/* Reading Button */}
            {selectedCards.length === 3 && !isReading && <div className="flex justify-center animate-scale-in">
                <Button size="lg" onClick={startReading} className="group">
                  Rivela la Saggezza
            ...
            {/* Reading Results */}
            {isReading && <div className="space-y-12 animate-fade-in">
                <div className="flex items-center justify-center gap-6">
                  {selectedCards.map((cardIndex, idx) => <div key={cardIndex} className="relative w-28 aspect-[2/3] minimal-border overflow-hidden animate-scale-in" style={{
                animationDelay: `${idx * 0.2}s`
              }}>
                      <img src={tarotCardBack} alt={`Carta selezionata ${cardIndex + 1}`} className="w-full h-full object-cover" />
                    </div>)}
                </div>
