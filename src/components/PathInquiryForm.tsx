...
const inquirySchema = z.object({
  ...
  email: z.string().trim().email("Inserisci un indirizzo email valido.").max(254, "L’email è troppo lunga.").toLowerCase(),
...
export function PathInquiryForm({ path, selectedTopic }: PathInquiryFormProps) {
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    if (!isPrivacyContactConfigured || submittingRef.current) return;
    ...
    form.reset({
      ...
      setSubmissionState("error");
      setStatusMessage("Connessione non disponibile. I dati sono rimasti nel form: riprova tra poco.");
    } finally {
      submittingRef.current = false;
    }
  }, () => {
    setSubmissionState("error");
    setStatusMessage("Controlla i campi indicati e conferma di aver letto l’informativa privacy.");
  });
    ...
    <section id="richiesta" className="relative scroll-mt-24 px-6 py-20 md:py-28" aria-labelledby={`${path}-inquiry-title`}>
      <div className="mx-auto max-w-3xl">
        <div className="sacred-card p-7 md:p-10">
          ...
          <form onSubmit={onSubmit} noValidate className="space-y-6" aria-busy={submissionState === "submitting"}>
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor={`${path}-name`}>Nome</Label>
                <Input id={`${path}-name`} aria-invalid={Boolean(form.formState.errors.name)} aria-describedby={form.formState.errors.name ? `${path}-name-error` : undefined} autoComplete="name" maxLength={100} className={inputClass} disabled={!isPrivacyContactConfigured || submissionState === "submitting"} {...form.register("name")} />
                {form.formState.errors.name && <p id={`${path}-name-error`} role="alert" className="text-sm text-destructive">{form.formState.errors.name.message}</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor={`${path}-email`}>Email</Label>
                <Input id={`${path}-email`} aria-invalid={Boolean(form.formState.errors.email)} aria-describedby={form.formState.errors.email ? `${path}-email-error` : undefined} type="email" inputMode="email" autoComplete="email" maxLength={254} className={inputClass} disabled={!isPrivacyContactConfigured || submissionState === "submitting"} {...form.register("email")} />
                {form.formState.errors.email && <p id={`${path}-email-error`} role="alert" className="text-sm text-destructive">{form.formState.errors.email.message}</p>}
              </div>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input id={`${path}-phone`} aria-invalid={Boolean(form.formState.errors.phone)} aria-describedby={form.formState.errors.phone ? `${path}-phone-error` : undefined} type="tel" inputMode="tel" autoComplete="tel" maxLength={30} className={inputClass} disabled={!isPrivacyContactConfigured || submissionState === "submitting"} {...form.register("phone")} />
                {form.formState.errors.phone && <p id={`${path}-phone-error`} role="alert" className="text-sm text-destructive">{form.formState.errors.phone.message}</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor={`${path}-topic`}>Motivo della richiesta</Label>
                <select id={`${path}-topic`} aria-invalid={Boolean(form.formState.errors.topic)} aria-describedby={form.formState.errors.topic ? `${path}-topic-error` : undefined} className={`flex h-10 w-full rounded-md border px-3 py-2 text-sm text-foreground outline-none ring-offset-background focus:ring-2 ${inputClass}`} disabled={!isPrivacyContactConfigured || submissionState === "submitting"} {...form.register("topic")}>
                  <option value="">Scegli un motivo</option>
                  {topics.map((topic) => <option key={topic.value} value={topic.value}>{topic.label}</option>)}
                </select>
                {form.formState.errors.topic && <p id={`${path}-topic-error`} role="alert" className="text-sm text-destructive">{form.formState.errors.topic.message}</p>}
              </div>
            </div>

            <div className="space-y-2">
              <Label htmlFor={`${path}-message`}>Messaggio</Label>
              <Textarea id={`${path}-message`} aria-invalid={Boolean(form.formState.errors.message)} aria-describedby={form.formState.errors.message ? `${path}-message-error` : undefined} rows={6} minLength={20} maxLength={2000} className={inputClass} disabled={!isPrivacyContactConfigured || submissionState === "submitting"} {...form.register("message")} />
              <div className="flex justify-between gap-4 text-xs text-muted-foreground">
                <span id={`${path}-message-error`} role="alert">{form.formState.errors.message?.message}</span>
            ...
            <div className="flex items-start gap-3">
              <Checkbox
                id={`${path}-privacy`}
                aria-invalid={!!form.formState.errors.privacyAccepted}
                aria-describedby={form.formState.errors.privacyAccepted ? `${path}-privacy-error` : undefined}
              ...
              <div className="space-y-1">
                ...
                {form.formState.errors.privacyAccepted && <p id={`${path}-privacy-error`} role="alert" className="text-sm text-destructive">{form.formState.errors.privacyAccepted.message}</p>}
              </div>
            </div>
